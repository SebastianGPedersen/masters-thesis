...
Rcpp::NumericVector sigmas_cpp(Rcpp::NumericVector dY, Rcpp::NumericVector kernels, int lags, double bandwidth) {
  ...
  /*
  //Calculate sum_kernel for the first lags+4 (will be used later)
  Rcpp::NumericVector sum_kernel(lags+4);
  
  sum_kernel[0] = kernels[0];
  for (int i = 1; i < lags+5; i++) {
    sum_kernel[i] = sum_kernel[i-1]*kernels[i];
  }
  ...
  //First weights
  first_weights[1] = parzen_weights[lags-1]; // because [i-4] is subtracted
  first_weights[2] = -4*parzen_weights[lags-1] + parzen_weights[lags-2];
  first_weights[3] = 6*parzen_weights[lags-1]-4*parzen_weights[lags-2] + parzen_weights[lags-3];
  first_weights[4] = -4*parzen_weights[lags-1]+6*parzen_weights[lags-2]-4*parzen_weights[lags-3] + parzen_weights[lags-4];
  ...
  middle_weights[1] = parzen_weights[lags/2] - temp1; //Extract the wrong side, add the right
  middle_weights[2] = -4*parzen_weights[lags/2] + 4*temp1-parzen_weights[lags/2-1]+temp2;
  middle_weights[3] = 6*parzen_weights[lags/2]-6*temp1 -4*parzen_weights[lags/2-1]+4*temp2+parzen_weights[lags/2-2]-temp3;
  middle_weights[4] = -4*parzen_weights[lags/2]+4*temp1 + 6*parzen_weights[lags/2-1]-6*temp2 -4*parzen_weights[lags/2-2]+4*temp3+parzen_weights[lags/2-3]-temp4;
  
  //Last weights
  last_weights[4] = 2; 
  last_weights[3] = parzen_weights[1] -4*2;
  last_weights[2] = parzen_weights[2] -4*parzen_weights[1]+6*2;
  last_weights[1] = parzen_weights[3]  -4*parzen_weights[2]+6*parzen_weights[1]-4*2;
  
  
  // First calculate weights(L+1,L+2,L+3,L+4), these are needed in the final loop 
  for (int i=0; i < 4; i++) {
    for (int k=0; k<lags; k++) {
      weights[lags+i] += sum_kernel[lags+i-(k+1)]*dY[lags+i-(k+1)]*parzen_weights[k];
    }
    weights[lags+i] += 2*dY[lags+i]*sum_kernel[lags+i]; //Include current time-point in weights
  }
