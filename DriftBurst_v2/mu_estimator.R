...
for (memory in 1:n_loops) {
  ...
  for (k_n in 1:length(k_n_list)) {
    ...
    print(length(rep(0,k_n_list[k_n]-2)))
    print(length(est.NewPreAverage(dy[,1],k_n_list[k_n])))
    ...
    mu_estimator_rescaled[((memory-1)*temp_paths+1):(memory*temp_paths),k_n] <- sqrt(h_mu) * mu_hat / sqrt(K2*sigma^2)
  }
  
}
T_estimator <- mu_estimator_rescaled
