...
kernel_function <- function(k_n,begin_index,all_time_points) {
  ...
  weights[begin_index+1:min(length(x_es),length(weights)-begin_index)] <- g_s
  return(weights)
}
...
### All the individual
time_points <- 0:30
begin_indices <- 1:(length(time_points)-1)
first_try <- kernel_function(k_n, 1, time_points)

tmp <- list()
for (i in 1:length(begin_indices)) {
  tmp[[i]] <- data.frame(
    x_akse = time_points,
    y_akse = kernel_function(k_n, begin_indices[i], time_points),
    grp = i,
    trans = 0.3+0.7*i/length(begin_indices)
  )
}
