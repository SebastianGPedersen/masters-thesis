...
n <- 23400
theta_list <- 1/sqrt(n)*c(5, 10, 25, 50, 75, 100, 125, 150, 175, 200, 225)

#Initialize list with 5 mean, lower and upper for later plot
all_plot_data <- vector("list", 5)
for (i in 1:5) {
  all_plot_data[[i]]$means <- numeric(length = length(theta_list))
...
for (my_theta in 1:length(theta_list)) {
  #my_n <- 1
  #my_n <- length(n_list)
  mat <- 6.5/(24*7*52)#*52*7*24*60*60 #In years
  dt <- mat/n #In years
  theta <- theta_list[my_theta]
  ...
  desired_index2 <- floor(n/2)
  
  ############ Simulation #########
  Npath <- 300
  settings <- sim.setup(mat=mat, Npath = Npath, Nsteps = n, omega = 0) #6.5 hours
  ...
  for (j in 1:length(all_paths)) {
    ...
    if(j == 2 || j == 3){
      ...
      for (i in 1:Npath){
        ...
        mu_hat <- est.mu(data = single_path, hd,t.index = desired_index2)$mu[1]
        sigma_hat_2 <- est.sigma.raw(data = single_path, hd,t.index = desired_index2)$sig[1]
        T_hat[i] <- sqrt(hd/K2) * mu_hat/sqrt(sigma_hat_2) #Correct
      }
    }
    ...
    all_plot_data[[j]]$means[my_theta] <- mean(T_hat_clean)
    all_plot_data[[j]]$lower[my_theta] <- all_plot_data[[j]]$means[my_theta] -sqrt(var(T_hat_clean))
    all_plot_data[[j]]$upper[my_theta] <- all_plot_data[[j]]$means[my_theta] +sqrt(var(T_hat_clean))
  }
}
...
all_plot_data[[2]]$factor <- rep("Drift burst, prev-average", length(all_plot_data[[2]]$mean))
all_plot_data[[3]]$factor <- rep("Jump, prev-average", length(all_plot_data[[3]]$mean))
...
plot_data_frame <- data.frame(theta = theta_list ,
...
for (i in 2:length(all_plot_data)){
  new_data_frame <- data.frame(theta = theta_list,
...
qplot(theta, mean, data = plot_data_frame, geom = "line", color = factor) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = factor), alpha = 0.3) +
  ggtitle("Convergence as pre-average interval increases")
