...
data.TforId <- function(data, id_name, id_number, hd, t.freq, lag, offset, offset_perId = FALSE){
  # Estimates T test for every t.freq (unit: seconds) within a certain id
  # hd and lag are parameters for estimation
...
# REWRITE data.TforId such that it does something by Id (check data table.intro)
# Throw away Ids that are not used in the function, then always do it by id (all ids)
# Rename .ids to something less dotty
# ALLOW FOR HD/HV
