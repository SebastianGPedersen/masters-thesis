######### SIM NOISE FUNCTION
vol.sim.simAR<-function(n, rho, sd){
  X<-rep(NA,n)
...
vol.est.IVestPathwise <- function(DT, cols = "logPrice", maxId){
  ...
  return(list(IVest = c(simpleIVList, advIVList), omega2est = omega2est))
}
...
vol.est.RVstar<-function(paData_subset, K, omega2Est, theta=1){
  ...
  return((N/(N-K+2)) * (1/(K*phiK)) * sum(paData_subset^2) - omega2Est/(theta*phiK))
}
